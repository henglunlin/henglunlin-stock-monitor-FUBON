...
with st.expander("ℹ️ 訊號邏輯與實測調整說明", expanded=False):
    st.markdown(
        f"""
- **訊號邏輯（四種判斷標準，彼此獨立，可同時出現）**：
  - **🔥 極早期訊號**：最新一筆連續兩筆都是外盤買，且單筆跳動或 2 秒漲幅已達標。最快、雜訊也最多，僅供提早注意。
  - **⚠️ 預警**：量能達標 ＋ 外盤占比達標 ＋ 2/5/10 秒任一短線漲幅達標。
...
- **📊 依實測 8883 筆訊號 log 分析調整**：
  - **極早期訊號**：原佔比 43% 但 15 分鐘後續轉正比例僅 23.8%（中位數 0%），已加強為需連續兩筆買盤 ＋ 獨立冷卻。
  - **低點反彈**：原佔比 46% 但 1~2% 反彈區間後續轉正僅 53%，已將門檻由 2.0% 提高到 3.0%、冷卻由 60 秒拉長到 240 秒（原本 49% 的重複觸發間隔都在 65 秒內，等於冷卻一過就馬上再觸發）。
  - **區間動能**：經驗證是品質最好的獨立訊號（15 分鐘後續轉正 67.4%，5 分鐘內轉化成預警 / 進場的比例 21.5%，遠高於極早期 3.9% 與低點反彈 9.1%），維持原本門檻。
        """
    )
st.caption(
